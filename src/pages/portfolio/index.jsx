...
const ImageCard = memo(function ImageCard({ img, description, link }) {
  ...
  return (
    <article className={`po_item ${loaded ? "is-loaded" : ""}`}>
      <a href={link} target="_blank" rel="noreferrer" className="po_link" aria-label="Открыть проект">
        <img
          src={img}
          alt={description}
          loading="lazy"
      ...
      <Container className="About-header">
        ...
        <section className="po_items_ho" aria-label="Список проектов">
          {dataportfolio.map((it, i) => (
            <ImageCard key={i} img={it.img} description={it.description} link={it.link} />
          ))}
        </section>
      </Container>
    </HelmetProvider>
